      ...
      <div className="w-full h-screen flex flex-col lg:flex-row lg:justify-around gap-8 text-center p-4">
        <section class="max-w-screen-sm mx-auto lg:max-w-xxl rounded-lg p-2 bg-[#ce93d8] self-start">
          ...
          <form class='p-2 mt-6'>
            ...
            <textarea rows={4} class='block w-full text(base gray-900) p-2 bg-[#ce93d8] rounded-lg border-4 border-[#ac5eb9dc] focus:outline-none placeholder-gray-500' placeholder='Write new memory here...' value={editMemoryPopUp ? '' : inputedMemory.content || ""} onChange={({currentTarget})=> setInputedMemory({...inputedMemory, content: currentTarget.value})}>
            </textarea>
